...
export const Home = ({ onLogout, onSalonSelect, onNavigate }: HomeProps) => {
  ...
  return (
    <div className="bg-[#FAFAFA] min-h-screen pb-24 font-sans">
      ...
      {/* HEADER */}
      <div className="bg-white p-6 pb-2 rounded-b-[32px] shadow-sm mb-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            onClick={onLogout}
            className="w-8 h-8 flex items-center justify-center bg-slate-50 text-slate-400 rounded-full hover:bg-red-50 hover:text-red-400 transition-colors"
          >
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl mb-6 relative">
          ...
          <button 
             ...
             className={`flex-1 py-3 text-xs font-bold rounded-lg transition-all ${
                activeTab === 'home' 
                ? 'bg-white text-[#00D4FF] shadow-sm' 
                : 'text-slate-400 hover:text-slate-600'
            }`}
          >
            At Home
        ...
        <h1 className="text-2xl font-bold text-slate-900 mb-1">
            {activeTab === 'salon' ? 'Good morning, Alex.' : 'Good morning, Julia.'}
        </h1>
        <p className="text-slate-400 text-sm mb-4">
            {activeTab === 'salon' ? 'Ready for your glow up?' : 'Ready for your session?'}
        </p>
      </div>
      ...
      {activeTab === 'salon' ? (
        <>
            <div className="pl-6 mb-8 animate-fade-in">
                ...
                <h2 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                    <span className="text-[#8B31FF]">✨</span> AI Picks For You
                </h2>
                <span className="text-xs font-bold text-[#00D4FF]">View all</span>
                </div>
            ...
            <div className="px-6 mb-8 animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {[{ name: 'Hair Styling', icon: '✂️', bg: 'bg-purple-100', color: 'text-purple-600' }, { name: 'Spa & Relax', icon: '🌿', bg: 'bg-cyan-100', color: 'text-cyan-600' }, { name: 'Makeup', icon: '💄', bg: 'bg-pink-100', color: 'text-pink-600' }, { name: 'Wellness', icon: '🧘‍♀️', bg: 'bg-green-100', color: 'text-green-600' }].map((cat) => (
                        <div key={cat.name} className="bg-white p-3 rounded-xl border border-slate-100 flex items-center gap-3">
            ...
            <div className="px-6 animate-fade-in">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="font-bold text-lg text-slate-900">Near You</h2>
                ...
                <div onClick={onSalonSelect} className="cursor-pointer active:scale-95 transition-transform">
                    <SalonCard name="Zenith Aesthetics" address="1.2 km • Closing soon" rating={4.8} tags={['Skin', 'Laser']} image="https://images.unsplash.com/photo-1629909613654-28e377c37b09?auto=format&fit=crop&q=80&w=200" />
                </div>
            </div>
        </>
      ) : (
        <div className="px-6 animate-fade-in">
            <div className="flex items-center gap-2 mb-4">
                <span className="text-purple-500 font-bold tracking-widest text-sm">BELYX</span>
                <span className="text-slate-900 font-bold text-lg">AI Recommended</span>
            </div>
            
            <HeroCard 
                title="Manicure Spa"
                subtitle="Based on your visit 3 weeks ago"
                rating={4.9}
                reviews={120}
                time="45m"
                ...
                <div className="flex justify-between">
                    {[{ name: 'Hair', icon: '✂️' }, { name: 'Massage', icon: '🌿' }, { name: 'Nails', icon: '💅' }, { name: 'Facial', icon: '🎭' }, { name: 'Yoga', icon: '🧘‍♀️' }].map((s) => (
                        <div key={s.name} className="flex flex-col items-center gap-2 group cursor-pointer">
                            <div className="w-14 h-14 bg-white rounded-[20px] shadow-sm border border-slate-100 flex items-center justify-center text-2xl group-hover:border-purple-200 group-hover:scale-110 transition-all">
                                {s.icon}
                            </div>
            ...
            <div>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="font-bold text-lg text-slate-900">Pros near you</h2>
